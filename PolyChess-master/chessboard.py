...
#==============================================================================
    def prise_en_passant(self, origine, destination):
        origine_ligne, origine_colonne = origine
        destination_ligne, destination_colonne = destination
        
        pion = self.get_piece(origine_ligne, origine_colonne)
        if not pion or pion.type != 'pion':
            return False
        
        # Vérifier si la destination est la case où le pion peut prendre en passant
        dernier_coup = self.dernier_coup
        if not dernier_coup:
            return False
        
        if not (destination_ligne == dernier_coup[1][0] and abs(destination_colonne - dernier_coup[1][1]) == 1):
            return False
        
        # Effectuer la prise en passant
        self.deplacer_piece(origine_ligne, origine_colonne, destination_ligne, destination_colonne)
        self.enlever_piece(dernier_coup[1][0], dernier_coup[1][1])  # Supprimer le pion pris en passant
