...
def roque(self, couleur, petit_roque=True):
    if couleur == 'blanc':
        roi = 'E1'
        tour = 'H1' if petit_roque else 'A1'
    else:
        roi = 'E8'
        tour = 'H8' if petit_roque else 'A8'

    if self.roquePossible(couleur, petit_roque):
        cases = ['F1', 'G1'] if petit_roque else ['D1', 'C1', 'B1']
        if all(self.get_piece(self.nomCaseToIndex(case)) is None for case in cases):
            for case in cases[:-1]:
                if self.caseAttaquee(case, couleur):
                    return False
            self.deplacerPiece(roi, cases[-1])
            self.deplacerPiece(tour, 'F1' if petit_roque else 'D1')
            return True
    return False
